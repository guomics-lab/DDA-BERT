...
from ..common.eval_model import Evalute
...
from ..transformer.dataset import SpectrumDataset
from ..transformer.dataset import collate_batch_weight_deltaRT_index
from ..transformer.model import DDA_BERT
from ..common_config import get_model_config_path


class PredictProcessHandler(CommonProcessHandler):

    def __init__(self, f_info: MzMLFileInfo, base_output_path, model_path, logger, env, input_param, start_time=0, open_sage=False,
        ...
        self.base_config = get_model_config_path()
    ...
    def deal_sage_pred(self, config, mzml_info):
        ...
        pred_ipc_dir = os.path.join(self.sage_gen_pred_ipc_result_dir, 'pred')
        ipc_path_list = os.listdir(pred_ipc_dir)
        ...
        for ipc_path in ipc_path_list:
            self.evaluate(os.path.join(pred_ipc_dir, ipc_path), config, mzml_info.base_file_name, sage_pred_dir)
    ...
    def evaluate(self, ipc_path, config, base_file_name, result_dir):
        ...
        for root, dirs, files in os.walk(result_dir):
            for file_name in files:
                if file_name == f'{base_file_name}_batch_pred.csv':

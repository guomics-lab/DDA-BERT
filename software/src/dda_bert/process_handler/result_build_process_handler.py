...
class ResultBuildProcessHandler(CommonProcessHandler):
    ...
    def deal_process(self):
        ...
        try:
            ...
            peptide_protein_df.rename(columns={'psm_id': 'PSMID',
                                               'sequence': 'PeptideSequence',
                                               'protein': 'ProteinGroup',
                                               'razor': 'Razor',
                                               'decoy_protein': 'DecoyProtein',
                                               'n_possible_proteins': 'nPossibleProteins',
                                               # 'index_protein': 'IndexProtein',
                                               'score_protein': 'ScoreProtein',
                                               'target': 'Target',
                                               'target_cum': 'TargetCum',
                                               'decoys_cum': 'DecoysCum',
                                               'fdr': 'FDR',
                                               'q_value_protein': 'ProteinQvalue'}, inplace=True)
            protein_save_col = ['FileName', 'PeptideSequence', 'ProteinGroup', 'ProteinQvalue', 'Razor', 'DecoyProtein',
                                'nPossibleProteins', 'ScoreProtein', 'Target', 'TargetCum', 'DecoysCum',
                                'FDR']
            ...
            fdr_df['PrecursorID'] = fdr_df['precursor_charge'].astype(str) + '_' + fdr_df['modified_sequence'].astype(
                str)
            ...
            psm_save_col = ['FileName', 'ScanNumber', 'PrecursorID', 'PrecursorMz',
                            'ModifiedSequence', \
                            'PrecursorCharge', 'Qvalue']

...
from ..common_config import get_work_fow_config_path
...
class FragpipeProcessHandler(CommonProcessHandler):
    ...
    def build_workflow(self, decoy_fast_path):
        ...
        if not self.fp_workflow_file:
            self.send_msg('Custom fragpipe workflow not set, use default workflow.')
            self.fp_workflow_file = get_work_fow_config_path()

        with open(self.fp_workflow_file, 'r', encoding='utf-8') as fin:
            content = fin.read()

...
def get_top_free_max_device(gpu_device_list):
    gpu_usages = get_gpu_usage()
    gpu_usages = [nn for nn in gpu_usages if nn['deviceID'] in gpu_device_list]
    gpu_usages.sort(key=lambda x: x['memoryFree'], reverse=True)
    return gpu_usages[0]['deviceID']

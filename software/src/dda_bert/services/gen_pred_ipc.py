...
def gen_sage_psm(psm, candidate, out_dir, file_name):
    ...
    pred_ipc_dir = os.path.join(out_dir, 'pred')
    os.makedirs(pred_ipc_dir, exist_ok=True)

    batch_size = 50000
    for i in range(0, len(psm), batch_size):
        batch = psm[i:i+batch_size]
        batch.write_ipc(os.path.join(pred_ipc_dir, f"batch_{i//batch_size}.ipc"))

    psm = psm.drop(['mz_array', 'intensity_array'])

...
def process_file(base_file_name, sage_ipc, ap_ipc, fp_ipc, sage_pred, ap_pred, fp_pred, output_dir) -> pl.DataFrame:
    ...
    save_target_fdr_psm = target_fdr_psm.drop(['mz_array', 'intensity_array'])
    ...
    df = df.drop(['mz_array', 'intensity_array'])
...
def deal_process(base_file_name, sage_ipc, ap_ipc, fp_ipc, sage_pred, ap_pred, fp_pred, output_dir):

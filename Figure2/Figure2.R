...
##Figure2A: PSMs barplot (cell line)
df <- read_excel('E:\\github\\DDA-BERT\\DDA-BERT_figure_2024.xlsx', sheet = "cell-line")
data_summary <- df %>% 
  group_by(FDR, Tool) %>% 
  summarise(
    mean_PSMs = mean(PSMs),
...
ggsave(filename = "E:\\github\\DDA-BERT\\psm_barplot\\Figure2_cell-line_PSMs.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
ggsave(filename = "E:\\github\\DDA-BERT\\precursor_barplot\\Figure2_cell-line_precursor_barplot.pdf", 
       plot = last_plot(), 
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2C: peptide barplot (cell line)
...
ggsave(filename = "E:\\github\\DDA-BERT\\peptide_barplot\\Figure2_cell-line_peptide_barplot.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2D: protein group barplot (cell line)
...
ggsave(filename = "E:\\github\\DDA-BERT\\pro_barplot\\Figure2_cell-line_pro_barplot.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2E: Venn diagram of protein groups (cell line)
library(eulerr)
...
data_dda_bert <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\dda_bert", "220627_GG")
data_sage <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\sage", "220627_GG")
data_fp <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\fp", "220627_GG")

list_dda_bert <- data_dda_bert$protein_group
list_sage <- data_sage$protein_group
list_fp <- data_fp$protein_group

only_dda_bert <- length(setdiff(list_dda_bert, union(list_sage, list_fp)))  # DDA_BERT_unique
only_sage <- length(setdiff(list_sage, union(list_dda_bert, list_fp)))      # Sage_unique
only_fp <- length(setdiff(list_fp, union(list_dda_bert, list_sage)))        # FragPipe_unique
...
ggsave("E:\\github\\DDA-BERT\\pro_venn\\figure2_cell-line_pro_venn.pdf", plot = p, device = "pdf", width = 1.7, height = 1.7)



##Figure2F: peptide_class_per_protein (cell line)
rm(list=ls())
...
ggsave(filename = "E:\\github\\DDA-BERT\\peptide_class_per_protein\\Figure2_cell-line_peptide_class_per_protein.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2G: PSMs score distributions (cell line)
library(ggplot2)
library(ggExtra)

data <- read.csv("E:\\DDA-BERT\\score_distribution\\220627_GG.csv")
...
ggsave(filename = "E:\\github\\DDA-BERT\\psm_score_distribution\\Figure2_cell-line_PSMs_score.pdf", 
       plot = p_with_density,
       device = "pdf",
       width = 4,
       height = 4,
...
##Figure2H: PSMs barplot (tissue)
rm(list=ls())
df <- read_excel('E:\\github\\DDA-BERT\\DDA-BERT_figure_2024.xlsx', sheet = "tissue")
...
ggsave(filename = "E:\\github\\DDA-BERT\\psm_barplot\\Figure2_tissue_PSMs.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2I: precursor barplot (tissue)
tissue_data <- figure2B %>% filter(Sample_type == "CRC FFPE samples")
...
ggplot(data_summary, aes(x = Tool, y = mean_Proteins / 10000, fill = Tool)) +
  ...
  scale_y_continuous(
    breaks = seq(0, 7.5, by = 1.5), 
    labels = c("0","1.5", "3.0", "4.5", "6.0", "7.5"), 
    limits = c(0, 7.5),
    expand = c(0, 0)
  )

ggsave(filename = "E:\\github\\DDA-BERT\\precursor_barplot\\Figure2_tissue_precursor_barplot.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2J: peptide barplot (tissue)
tissue_data <- figure2B %>% filter(Sample_type == "CRC FFPE samples")
...
ggsave(filename = "E:\\github\\DDA-BERT\\peptide_barplot\\Figure2_tissue_peptide_barplot.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2K: protein group barplot (tissue)
tissue_data <- figure2B %>% filter(Sample_type == "CRC FFPE samples")
...
ggsave(filename = "E:\\github\\DDA-BERT\\pro_barplot\\Figure2_tissue_pro_barplot.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2L: Venn diagram of protein groups (tissue)
library(eulerr)
rm(list=ls())
read_and_merge_with_info <- function(directory, pattern) {
  files <- list.files(directory, pattern = pattern, full.names = TRUE)
...
data_dda_bert <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\dda_bert", "20210715_Exploris1")
data_sage <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\sage", "20210715_Exploris1")
data_fp <- read_and_merge_with_info("E:\\DDA-BERT\\peptide_per_protein\\fp", "20210715_Exploris1")
...
ggsave("E:\\github\\DDA-BERT\\pro_venn\\figure2_tissue_pro_venn.pdf", plot = p, device = "pdf", width = 1.7, height = 1.7)



##Figure2M: peptide_class_per_protein (tissue)
rm(list=ls())
process_file_data <- function(file_data, cleaned_sequence_col) {
  if (!cleaned_sequence_col %in% colnames(file_data)) {
    stop(paste("Error: Specified 'cleaned_sequence' column not found in the file. Expected column:", cleaned_sequence_col))
...
ggsave(filename = "E:\\github\\DDA-BERT\\peptide_class_per_protein\\Figure2_tissue_peptide_class_per_protein.pdf", 
       plot = last_plot(),
       device = "pdf",
       width = 2.3,
       height = 2.3, 
...
##Figure2N: PSMs score distributions (tissue)
rm(list=ls())
data <- read.csv("E:\\DDA-BERT\\score_distribution\\20210715_Exploris1_SA_NBext_LW_darm_FFPE.csv")
data$label <- ifelse(data$label == 1, "Target", "Decoy")
...
ggsave(filename = "E:\\github\\DDA-BERT\\psm_score_distribution\\Figure2_tissue_PSMs_score.pdf", 
       plot = p_with_density,
       device = "pdf",
       width = 4,
       height = 4,
